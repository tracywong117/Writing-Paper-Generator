...
const spacingOptions = computed(() => {
  if (props.settings.paperType === 'line') {
    return [6, 7, 8, 9, 10]
  }
  return [3, 4, 5, 6]
})
...
<template>
  <div class="sidebar" :class="{ open: isOpen }" id="sidebar">
    ...
    <div class="control-section">
      <h2 class="heading-serif">Paper</h2>
      <div class="control-group">
        <label for="paperType">Type</label>
        <select id="paperType" v-model="settings.paperType">
      ...
      <div class="control-group">
        ...
        <select id="spacing" v-model.number="settings.spacing">
          <option v-for="opt in spacingOptions" :key="opt" :value="opt">{{ opt }}mm</option>
        </select>
      </div>
      <div class="control-group">
        <label for="pageSize">Size</label>
        <select id="pageSize" v-model="settings.pageSize">
    ...
    <div class="control-section">
      <h2 class="heading-serif">Colors</h2>
      <div class="control-group color-picker">
        <label for="bgColor">Background</label>
        <select id="bgColor" v-model="settings.bgColor">
          <option value="#FFFFFF">White</option>
          <option value="#FAFAF5">Ivory</option>
          <option value="#F5F5F0">Warm Gray</option>
          <option value="custom">Custom</option>
        </select>
        <input 
          v-if="settings.bgColor === 'custom'" 
          type="color" 
          v-model="settings.customBgColor"
        >
      </div>
      <div class="control-group color-picker">
        <label for="lineColor">Lines / Dots</label>
        <select id="lineColor" v-model="settings.lineColor">
          <option value="#E5E5E2">Light Lead</option>
          <option value="#CCCCCC">Medium Gray</option>
          <option value="#7A7A72">Graphite</option>
          <option value="custom">Custom</option>
        </select>
        <input 
          v-if="settings.lineColor === 'custom'" 
          type="color" 
          v-model="settings.customLineColor"
        >
      </div>
    </div>

    <div v-if="settings.paperType === 'line'" class="control-section" id="lineOptions">
      <h2 class="heading-serif">Line Options</h2>
      <div class="control-group">
        <label for="headerHeight">Header Height (mm)</label>
        <input type="number" id="headerHeight" min="0" max="50" v-model.number="settings.headerHeight">
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="redLine" v-model="settings.redLine">
        <label for="redLine">Red Margin Line</label>
      </div>
      <div class="control-group" v-if="settings.redLine">
        <label for="redLinePadding">Margin Padding (mm)</label>
        <input type="number" id="redLinePadding" min="0" v-model.number="settings.redLinePadding">
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="titleSection" v-model="settings.titleSection">
        <label for="titleSection">Title Section</label>
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="noField" v-model="settings.noField">
        <label for="noField">No. Field</label>
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="dateField" v-model="settings.dateField">
        <label for="dateField">Date Field</label>
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="footer" v-model="settings.footer">
        <label for="footer">Footer Line</label>
      </div>
      <div class="control-group" v-if="settings.footer">
        <label for="footerStart">Footer Start (Line # from end)</label>
        <input type="number" id="footerStart" min="1" max="5" v-model.number="settings.footerStart">
      </div>
      <div class="control-group checkbox-wrapper">
        <input type="checkbox" id="ruler" v-model="settings.ruler">
        <label for="ruler">Ruler Marks</label>
      </div>
    </div>
...
@media (max-width: 768px) {
  .sidebar {
    ...
    position: fixed;
    top: 0;
    left: -320px; /* Slightly more than width to hide completely */
    bottom: 0;
